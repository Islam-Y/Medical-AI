...
class OutboxServiceTest {
    ...
    void enqueueSerializesEventPayload() {
        ...
        // Act
        service.enqueue("topic", EventTypes.DOCUMENT_UPLOADED, userId, EventEnvelope.create(EventTypes.DOCUMENT_UPLOADED, UUID.randomUUID(), userId, "payload"));

        // Assert
        ArgumentCaptor<OutboxEvent> captor = ArgumentCaptor.forClass(OutboxEvent.class);
        verify(repository).save(captor.capture());
        assertThat(captor.getValue().getPayload()).contains(EventTypes.DOCUMENT_UPLOADED);
    }
}
