...
class OutboxServiceTest {
    ...
    void enqueueSerializesEventPayload() {
        ...
        // Act
        service.enqueue("topic", EventTypes.HEALTH_RECORD_CHANGED, userId, EventEnvelope.create(EventTypes.HEALTH_RECORD_CHANGED, UUID.randomUUID(), userId, "payload"));

        // Assert
        ArgumentCaptor<OutboxEvent> captor = ArgumentCaptor.forClass(OutboxEvent.class);
        verify(repository).save(captor.capture());
        assertThat(captor.getValue().getPayload()).contains(EventTypes.HEALTH_RECORD_CHANGED);
    }
}
