...
class OutboxServiceTest {
    ...
    void enqueueSerializesEventPayload() {
        ...
        // Act
        service.enqueue("topic", EventTypes.USER_PROFILE_UPDATED, userId, EventEnvelope.create(EventTypes.USER_PROFILE_UPDATED, UUID.randomUUID(), userId, "payload"));

        // Assert
        ArgumentCaptor<OutboxEvent> captor = ArgumentCaptor.forClass(OutboxEvent.class);
        verify(repository).save(captor.capture());
        assertThat(captor.getValue().getTopic()).isEqualTo("topic");
        assertThat(captor.getValue().getPayload()).contains(EventTypes.USER_PROFILE_UPDATED);
    }
}
